    ...
    loop {
        if !game_over {
            ...
            if get_time() - last_update > speed {
                ...
                for (x, y) in &snake.body {
                    if *x == snake.head.0 && *y == snake.head.1 {
                        let current_pos = snake.body.iter().position(|&p| p == (*x, *y));

                        if let Some(index) = current_pos {
                            let snake_len = snake.body.len();
                            let _ = snake.body.split_off(index);

                            score -= ((snake_len - index) * 100) as i32;
                            break;
                        }
                    }
                }
            ...
            for i in 1..SQUARES {
                ...
                draw_text(format!("SCORE: {score}").as_str(), 10., 20., 20., WHITE);
            }
        } else {
            clear_background(LIGHTGRAY);
            let text = "You've lost! Press [enter] to try again";
            let font_size = 30.;
